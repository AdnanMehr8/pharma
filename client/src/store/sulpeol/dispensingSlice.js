...
export const dispensingSlice = createSlice({
    ...
    reducers: {
        setsDispensing: (state, action) => {
            ...
            localStorage.setItem('s-dispensing', JSON.stringify(state));
            console.log('Stored dispensing in local storage:', state);
        },
        resetsDispensing: (state) => {
            // Reset the state to initial values
            return initialState;
        },
        loadsDispensingFromStorage: (state) => {
            const record = JSON.parse(localStorage.getItem('s-dispensing'));
...
export const { setsDispensing, resetsDispensing, loadsDispensingFromStorage } = dispensingSlice.actions;
